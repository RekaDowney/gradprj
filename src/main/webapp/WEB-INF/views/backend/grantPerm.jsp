...
<div class="row">
    <div class="col-sm-11 pull-left">
        <ul id="permTree" class="ztree" style="min-width: 80%;height: 100%;"></ul>
    </div>
...
<script type="text/javascript">
    ...
    function beforeCheck(treeId, treeNode) {
        if (treeNode.name == '文档浏览') {
            var flag = treeNode.getCheckStatus();
            if (flag.checked) {
                layer.open({
                    title: '错误',
                    icon: 5,
                    content: '请确保所有的角色都拥有浏览权限',
                    shadeClose: true
                });
                return false;
            }
    ...
    function getTree(treeContainerId) {
        if (!treeContainerId) {
            treeContainerId = 'permTree';
        }
        return $.fn.zTree.getZTreeObj(treeContainerId);
    }

    $(function () {
        var tree = $.fn.zTree.init($("#permTree"), setting, zNodes);
        tree.expandAll(true);
        ...
        $('#submit').on('click', function () {
            ...
            if (nodes.length == 0) {
                layer.open({
                    ...
                    title: '错误',
                    icon: 5,
                    content: '请至少勾选“文档浏览”权限'
                });
                return;
            }

            var catIdArr = [];
            var containsViewFlag = false;
            for (var i = 0, node; node = nodes[i]; i++) {
                catIdArr.push(node.id);
                if (node.name == '文档浏览') {
                    containsViewFlag = true;
                }
            }
            if (!containsViewFlag) {
                layer.open({
            ...
            $.ajax({
                url: baseUrl + "/role/" + roleId + "/perm/grant",
                type: "POST",
